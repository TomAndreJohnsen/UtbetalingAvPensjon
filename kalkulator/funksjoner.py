...
def vis_meny(param):
    ...
    print(f"Fødselsdato:        {vis_verdi(param.person.fodselsdato):<18}   Gevinstskatt: {param.skatt.gevinst}")
    print(f"Alder:              {vis_verdi(param.person.alder):<18}   Formue kommune: {param.skatt.formue_kommune}")
    print(f"Ønsket netto:       {vis_verdi(param.uttak.maanedlig_netto):<18}   Formue stat lav: {param.skatt.formue_stat_lav}")
    print(f"Start pensjon:      {vis_verdi(param.person.pensjon_start):<18}   Formue stat høy: {param.skatt.formue_stat_hoy}")
    print(f"Slutt pensjon:      {vis_verdi(param.person.pensjon_slutt):<18}   Bunnfradrag: {param.skatt.formue_bunnfradrag}")
    print(f"Gjeld:              {vis_verdi(param.person.gjeld):<18}   Høy grense: {param.skatt.formue_grense_hoy}")
    print(f"Eiendom:            {vis_verdi(param.person.eiendom):<18}")
    print(f"Startkapital:       {vis_verdi(param.fond.startkapital):<18}")
    print(f"Månedlig innskudd:  {vis_verdi(param.fond.maanedlig_innskudd):<18}")
    print("\n------------------------\n")